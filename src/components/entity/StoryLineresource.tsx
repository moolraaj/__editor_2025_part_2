...
export const CreateStorylinePopup: React.FC<CreateStorylinePopupProps> = ({ onClose, onSubmit }) => {
  ...
  // ✅ Toggle function: sirf ek hi field open hoga
  const toggle = (key: keyof FormState) => {
    setOpen(prev => {
      const newState: Record<keyof FormState, boolean> = {
        title: false,
        challenge: false,
        turningPoint: false,
        introMain: false,
      ...
      newState[key] = !prev[key]; // Same field pe click kare to close bhi ho sakta
      return newState;
    });
  };
  ...
  const renderField = (field: keyof FormState, placeholder: string, multiline = false, rows = 2) => {
    return (
      <div className="relative mb-4">
  ...
  <div className="popup-container next-page-popup-container">
    ...
    {step === 0 && (
      <>
        <div className="popup-fields">
          {(['title', 'challenge', 'turningPoint', 'introMain', 'introSupp1', 'cta'] as (keyof FormState)[]).map((field, index) => {
            ...
            return (
              <div key={field} className="popup-field">
                <button onClick={() => toggle(field)} className="field-toggle-button flex items-center gap-2 relative">
                  {/* ✅ Dot showing filled or empty with number inside */}
                  <div className={`status-dot ${isFilled ? 'filled' : ''}`}>
