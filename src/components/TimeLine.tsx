...
export const TimeLine: React.FC = observer(() => {
  ...
  const handleSceneClick = (idx: number) => {
    ...
    console.groupEnd(); // end text group

    console.groupEnd(); // end “Active Scene” group

    // 4.10) If you still want to clear/highlight on the Fabric canvas, do that afterward
    if (store.canvas) {
  ...
  // 5) Sync activeSceneIndex/currentSceneIndex when time updates (master view)
  useEffect(() => {
    if (sceneTimings.length === 0) return;
    const ct = store.currentTimeInMs;
    // find index where ct ∈ [start,end)
  ...
  // 6) In “scene” view, loop that scene and pause when done
  useEffect(() => {
  ...
  // 8) Render all scenes side by side in master view
  const renderMasterView = () => {
    return (
      <div className="relative h-48" onDragOver={(e) => e.preventDefault()}>
        ...
        {/* Render any global (non‐scene) elements below */}
